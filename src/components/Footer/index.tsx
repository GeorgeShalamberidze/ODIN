        ...
        <div className="flex justify-between w-full pb-9 pt-9 3xs:pb-14 3xs:pt-14 gap-8">
          <div className="flex-col gap-8 flex-1 max-w-[470px] hidden 3xs:flex">
            ...
            <p className="hidden md:flex">
              ...
              delivery. By leveraging the unused luggage space of travellers, we
              offer a cost-effective, reliable, and secure way to send parcels
              across borders. Whether you're looking to save on shipping costs
              or earn while you travel, ODIN connects the world in a more
              meaningful way
            </p>
          </div>
          <div className="flex flex-col gap-3 flex-1 md:flex-none font-bold items-end 3xs:items-start">
            {headerItems.map((item) => (
              <div key={item.id} className="cursor-pointer hover:opacity-65">
                <p>{item.name}</p>
              </div>
            ))}
          </div>
